...
public:
    ...
    void Energy(SimulationParameters& parameter); ///< 计算跃迁能量
    void InitStatesArray(SimulationParameters& parameter); ///< 初始化晶胞
    void Periodic(SimulationParameters& parameter); ///< 处理周期性边界条件
    void RecycleFNeighbors(SimulationParameters& parameter); ///< 循环计算FCC近邻
    void RecycleBNeighbors(SimulationParameters& parameter); ///< 循环计算BCC近邻

    virtual void CountNeighbors(int nm, int i, int  j, int  k, std::vector<int> count1, std::vector<int> count2) = 0; ///< 计算近邻数

private:
    
    std::vector<int> CalculateN1Neighbors(SimulationParameters& parameter, int nm, int nmn, int i, int  j, int  k);  ///< 循环计算BCC第一近邻
    std::vector<int> CalculateN1FNeighbors(SimulationParameters& parameter, int nm, int nmn, int i, int  j, int  k); ///< 循环计算FCC第一近邻
    std::vector<int> CalculateN2Neighbors(SimulationParameters& parameter, int nm, int nmn, int i, int  j, int  k);  ///< 循环计算第二近邻
